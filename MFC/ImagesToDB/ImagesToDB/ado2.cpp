...
DWORD CADODatabase::GetRecordCount(_RecordsetPtr pRs)
{
	DWORD numRows = 0;
...
BOOL CADOParameter::GetValue(double& dbValue)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			//			dblVal /= pow(10.0, static_cast<double>(vtVal.decVal.scale));
			dblVal /= pow(10.0, static_cast<int>(vtVal.decVal.scale));
			break;
		case VT_UI1:
			dblVal = vtVal.iVal;
			break;
		...
		case VT_DECIMAL:
			{
				...
				val /= pow(10.0, static_cast<int>(vtVal.decVal.scale)); 
				strVal = DblToStr(val);
			}
			break;
		case VT_DATE:
