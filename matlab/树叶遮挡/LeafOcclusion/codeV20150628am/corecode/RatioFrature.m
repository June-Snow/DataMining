...
%%
maxImg = max(firstFrame,[],3);%求出每一个像素点的最大值，即RGB中的最大值
...
rCastLabel = diff(:,:,1) == 0;
rCastLabel = rCastLabel.^(double(~noCastLabel));
rCastRatio = mean(rCastLabel(:));
gCastLabel = diff(:,:,2) == 0;
gCastLabel = gCastLabel.^(double(~noCastLabel));
gCastRatio = mean(gCastLabel(:));
bCastLabel = diff(:,:,3) == 0;
bCastLabel = bCastLabel.^(double(~noCastLabel));
