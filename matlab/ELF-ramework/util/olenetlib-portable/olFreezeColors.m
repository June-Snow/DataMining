function olFreezeColors(varargin)
% olFreezeColors  Lock colors of plot, enabling multiple colormaps per figure. (v2.3)
%
%   Problem: There is only one colormap per figure. This function provides
%       an easy solution when plots using different colomaps are desired 
%       in the same figure.
%
%   olFreezeColors freezes the colors of graphics objects in the current axis so 
%       that subsequent changes to the colormap (or caxis) will not change the
%       colors of these objects. olFreezeColors works on any graphics object 
...
%       based on the colormap active at the time olFreezeColors is called.
%
...
%
%   Usage:
%       olFreezeColors        applies to all objects in current axis (gca),
%       olFreezeColors(axh)   same, but works on axis axh.
%
%   Example:
%       subplot(2,1,1); imagesc(X); colormap hot; olFreezeColors
%       subplot(2,1,2); imagesc(Y); colormap hsv; olFreezeColors etc...
%
%       Note: colorbars must also be frozen. Due to Matlab 'improvements' this can
%				no longer be done with olFreezeColors. Instead, please
...
%
%   Side effect on render mode: olFreezeColors does not work with the painters
%       renderer, because Matlab doesn't support rgb color data in
%       painters mode. If the current renderer is painters, olFreezeColors
...
%   olFreezeColors('nancolor',[r g b]) or olFreezeColors(axh,'nancolor',[r g b]),
...
for hh = cdatah',
    ...
    if strcmp(scalemode,'scaled'),
        ...
        idx = cdata;
        %10/8/09 in case direct data is non-int (e.g. image;olFreezeColors)
...
% checkArgs  Validate input arguments to olFreezeColors
...
if mod(nargs,2),
    h = args{1};
    if ~ishandle(h),
        error('JRI:olFreezeColors:checkArgs:invalidHandle',...
    ...
    if strcmp(get(h,'Tag'),'Colorbar'),
      if ~exist('cbfreeze.m'),
        warning('JRI:olFreezeColors:checkArgs:cannotFreezeColorbar',...
            ['You seem to be attempting to freeze a colorbar. This no longer'...
            'works. Please read the help for olFreezeColors for the solution.'])
      else
        cbfreeze(h);
        return
      end
...
if nargs == 2,
    if strcmpi(args{end-1},'nancolor'),
        ...
        if ~all(size(nancolor)==[1 3]),
            error('JRI:olFreezeColors:checkArgs:badColorArgument',...
                'nancolor must be [r g b] vector');
        end
        nancolor(nancolor>1) = 1; nancolor(nancolor<0) = 0;
    else
        error('JRI:olFreezeColors:checkArgs:unrecognizedOption',...
