...
for i=1:n
    ...
    switch l.type
        ...
        case 'regressionloss'%回归损失函数
            res(i+1).x = vl_nnregressionloss(res(i).x, l.class) ;
        case 'relu'
        ...
        case 'scoreloss'
            res(i+1).x = elf_scoreloss(res(i).x, l.class) ;
        case 'custom'
...
if doder
  ...
  for i=n:-1:1
    ...
    switch l.type
      ...
      case 'softmaxloss'%分类损失函数
        res(i).dzdx = vl_nnsoftmaxloss(res(i).x, l.class, res(i+1).dzdx) ;
      case 'regressionloss'%回归损失函数
        res(i).dzdx = vl_nnregressionloss(res(i).x, l.class, res(i+1).x) ;        
      case 'relu'%激活
